...
class NMT_GRU(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder=nn.GRUCell(input_size=embed_size+hidden_size,
                                 hidden_size=hidden_size,
                                 bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        @returns dec_init_state (Tensor)): Tensor representing the decoder's initial
                                        hidden state. Should have shape (b, h).
        """
        ...
        X = torch.permute(X,(1, 2, 0)) # (src_len, b, e) -> (b, e, src_len)
        X = self.post_embed_cnn(X)
        X = torch.permute(X,(2, 0, 1)) # (b, e, src_len) -> (src_len, b, e)
        ...
        X = pack_padded_sequence(input=X,
                                 ...
                                 enforce_sorted=True)
        enc_hiddens, last_hidden =self.encoder(X) # enc_hiddens (src_len, b, h*2)
        ...
        dec_init_state = init_decoder_hidden
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Ys = torch.split(Y, split_size_or_sections=1) # (tgt_len, b, e) -> tuple[tgt_len](b, e)
        for Y_t in Ys:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @param dec_state (Tensor): Tensor representing the decoder's initial hidden state.
                                Should have shape (b, h).
        ...
        # dec_hidden @ enc_hiddens_proj = (1, h) @ (h, src_len) = (b, 1, src_len)
        # (b, 1, src_len) -> (b, src_len)
        ###
        ###       Hints:
        ...
        # alpha_t @ enc_hiddens = (1, src_len) @ (src_len, 2h) = (b, 1, 2h)
        # (b, 1, 2h) -> (b, 2h)
    ...
    def load(model_path: str):
        ...
        args = params['args']
        model = NMT_GRU(vocab=params['vocab'], **args)
